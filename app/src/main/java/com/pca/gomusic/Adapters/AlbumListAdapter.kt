...
class AlbumListAdapter(var activity: Activity, var songs: ArrayList<Song>) : RecyclerView.Adapter<AlbumListAdapter.MyViewHolder>() {
    ...
    private val receiver: BroadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            if (UPDATE_ADAPTER_LIST == Objects.requireNonNull(intent.action)) {
                notifyDataSetChanged()
            } else if (DEACTIVATE_RECIEVER == Objects.requireNonNull(intent.action)) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        ...
        mainView = if (viewType == EMPTY_VIEW) {
            ...
            layoutInflater.inflate(R.layout.album_list_item, parent, false)
        }
                ...
                popupMenu.setOnMenuItemClickListener { item: MenuItem ->
                    when (item.itemId) {
                        ...
                        R.id.delete -> {
                            ...
                            val deleteBuilder = AlertDialog.Builder(Objects.requireNonNull(activity))
                            ...
                            deleteInflater.findViewById<View>(R.id.ok).setOnClickListener {
                                ...
                                if (songFile.delete()) {
                                    ...
                                    deleted.action = UPDATE_ADAPTER_LIST
    ...
    companion object {
        const val UPDATE_ADAPTER_LIST = "UPDATE_ADAPTER_LIST"
        private const val DEACTIVATE_RECIEVER = "DEACTIVATE_RECIEVER"
    }

    init {
        val filter = IntentFilter()
        filter.addAction(UPDATE_ADAPTER_LIST)
        filter.addAction(DEACTIVATE_RECIEVER)
